...
Room* g_pWelcomeCenter = nullptr;
Room* g_pClothingStore = nullptr;
Room* g_pMovieTheater = nullptr;
Room* g_pCandyShop = nullptr;
Room* g_pFoodCourt = nullptr;
...
int main()
{
    ...
    g_pWelcomeCenter = initWelcomeCenter();
    g_pClothingStore = initClothingStore();
    g_pMovieTheater = initMovieTheater();
    g_pCandyShop = initCandyShop();
    g_pFoodCourt = initFoodCourt();
    ...
    // display app version
    messageQueue.push_back("Welcome " + playerName + " to the Mall, and to the beginning of your adventure!");
...
// main app loop: display game menu, handle user command
    do
    {
        ...
        // G)uess Magic Word command
        else if (userCmd == 'm')
            guessMagicWord(pPlayer, pRoom, messageQueue);
        // I)nfo command
        else if (userCmd == 'i')
...
char menuOption()
{
    ...
    std::cout << "T)ake weapon, deF)end, drO)p weapon, guess M)agic word,\n";
