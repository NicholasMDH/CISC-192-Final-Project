...
* guessMagicWord()
*
* prompt user to input their guess, check their guess,
* transfer MagicWord from Room to Player, and add points
*******************************************************************************
*/
bool guessMagicWord(Player* pP, Room* pR, std::vector<std::string>& msgQ)
{
    //professor, I know you probably wouldn't like me doing this this way,
    //but I don't want to call another function just to avoid a couple of
    //cout's
    ...
    //prompt the user to input the magic word text
    std::cout << "Enter your guess for what the magic word is: \n";

    //clear input buffer
    std::cin.ignore();
    
    //Get user guess
    std::getline(std::cin, mText);
    pM->setWordGuess(mText);

    //compare the strings
    if (!pM->compareWordText()) //never compare to true
        std::cout << "Wrong guess\n";
    else
    {
        std::cout << "You guessed correctly\n";

        //Transfer Magic Word from Room to Player
        pP->addMagicWord(pM);
        std::string str;
        pM->getName(str);
        ...
        // build message string with string stream
        std::stringstream ss;
        ss << "You have a " << str << " worth " << pM->getPoints() << " points";
...
Room* movePlayer(Player* pP, Room* pR, int direction, std::vector<std::string>& msgQ)
{
    ...
    //Inform the user that there's nothing there
